...
static sFONT *LCD_Currentfonts = &Font24x32;  //Ӣ������
...
{
    HAL_GPIO_WritePin ( ILI9341_RST_PORT, ILI9341_RST_PIN,GPIO_PIN_RESET );	 //�͵�ƽ��λ
...
void ILI9341_FillImage (PFNUpdate UpdateProc)
{
    /* memory write */
    ILI9341_Write_Cmd ( CMD_SetPixel );

    for(uint32_t j=0;j<240;j++) {
        uint16_t * data = UpdateProc();
        if(!data)return;
 ...
 * @retval ��
 */
void ILI9341_SetPointPixel ( uint16_t usX, uint16_t usY ,uint16_t color)
{	
...
void ILI9341_DrawLine ( uint16_t usX1, uint16_t usY1, uint16_t usX2, uint16_t usY2 )
{
	...
	else 
  {
	...
	{  
		ILI9341_SetPointPixel ( usX_Current, usY_Current ,CurrentTextColor );//����
...
void ILI9341_DrawCircle ( uint16_t usX_Center, uint16_t usY_Center, uint16_t usRadius, uint8_t ucFilled )
{
	...
	while ( sCurrentX <= sCurrentY )
	{
		...
		if ( ucFilled ) 			
			...
			{                      
				ILI9341_SetPointPixel ( usX_Center + sCurrentX, usY_Center + sCountY,CurrentTextColor  );           //1���о�����
				ILI9341_SetPointPixel ( usX_Center - sCurrentX, usY_Center + sCountY ,CurrentTextColor );           //2
				ILI9341_SetPointPixel ( usX_Center - sCountY,   usY_Center + sCurrentX ,CurrentTextColor );           //3
				ILI9341_SetPointPixel ( usX_Center - sCountY,   usY_Center - sCurrentX ,CurrentTextColor );           //4
				ILI9341_SetPointPixel ( usX_Center - sCurrentX, usY_Center - sCountY,CurrentTextColor  );           //5
        ILI9341_SetPointPixel ( usX_Center + sCurrentX, usY_Center - sCountY,CurrentTextColor  );           //6
				ILI9341_SetPointPixel ( usX_Center + sCountY,   usY_Center - sCurrentX,CurrentTextColor  );           //7
        ILI9341_SetPointPixel ( usX_Center + sCountY,   usY_Center + sCurrentX ,CurrentTextColor );           //0
			}
		
		else
		{          
			ILI9341_SetPointPixel ( usX_Center + sCurrentX, usY_Center + sCurrentY,CurrentTextColor );             //1���о�����
			ILI9341_SetPointPixel ( usX_Center - sCurrentX, usY_Center + sCurrentY ,CurrentTextColor );             //2
			ILI9341_SetPointPixel ( usX_Center - sCurrentY, usY_Center + sCurrentX ,CurrentTextColor );             //3
			ILI9341_SetPointPixel ( usX_Center - sCurrentY, usY_Center - sCurrentX ,CurrentTextColor );             //4
			ILI9341_SetPointPixel ( usX_Center - sCurrentX, usY_Center - sCurrentY ,CurrentTextColor );             //5
			ILI9341_SetPointPixel ( usX_Center + sCurrentX, usY_Center - sCurrentY ,CurrentTextColor );             //6
			ILI9341_SetPointPixel ( usX_Center + sCurrentY, usY_Center - sCurrentX,CurrentTextColor  );             //7
			ILI9341_SetPointPixel ( usX_Center + sCurrentY, usY_Center + sCurrentX ,CurrentTextColor );             //0
    }			
 ...
 * @retval ��
 */
void ILI9341_DrawChar_Ex(uint16_t usX, //�ַ���ʾλ��x
												uint16_t usY, //�ַ���ʾλ��y
